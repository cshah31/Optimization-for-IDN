...
eff_out = 0.95 
r_batt = np.vstack((np.zeros((1,1)), 0.1 * np.random.rand(nl,1))) # Battery Energy Storage Internal Resistance
r_batt = np.array(r_batt.T)
r_batt = r_batt.ravel()
r_cvt = noh * np.vstack((np.zeros((1,1)), 0.1 * np.random.rand(nl,1)))  # Power Converter Resistance
r_cvt = np.array(r_cvt.T)
r_cvt = r_cvt.ravel()
p_battmax = np.max(fixed_load_p,axis=1)*0.5
p_battmax = np.array(p_battmax.T)
p_battmax = p_battmax.ravel()
q_battmax = np.max(fixed_load_q,axis=1)*0.5
q_battmax = np.array(q_battmax.T)
q_battmax = q_battmax.ravel()
s_battmax = np.sqrt(np.square(p_battmax) + np.square(q_battmax))
hours = 5
...
df13 = pd.DataFrame({"Nodes": no_of_nodes,
                    ...
                    "Q_battmax": -q_battmax.T,
                    "S_battmax": s_battmax.T,
                    "battsize": battsize.T,
                    "E_min": E_min.T,
                    "E_max": E_max.T
})
...
writer = pd.ExcelWriter("zone2_nonlinear_opt_model.xlsx", engine='xlsxwriter')
...
workbook.define_name('BESS', '=Sheet1!$BJ$1:$BR$18')
workbook.define_name('P_Shp_Load', '=Sheet1!$BT$1:$BW$18')
workbook.define_name('selectShp', '=Sheet1!$BY$2:$CW$19')
workbook.define_name('alpha', '=Sheet1!$CY$1:$CZ$18')
workbook.define_name('beta', '=Sheet1!$DB$2:$DZ$19')
workbook.define_name('Pg_min', '=Sheet1!$EB$2:$EZ$19')
workbook.define_name('Pg_max', '=Sheet1!$EB$22:$EZ$39')
...
workbook.define_name('V', '=Sheet1!$E$28')
...
model.r_batt = Param(model.Nodes)
model.r_cvt = Param(model.Nodes)
model.P_battmax = Param(model.Nodes)
model.Q_battmax = Param(model.Nodes)
model.S_battmax = Param(model.Nodes)
...
model.V = Param(mutable=True, initialize=4.16)
...
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="T", format='set', set='T')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="Nodes", format='set', set='Nodes')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="FT_Nodes", format='set', set='FT_Nodes')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="RX", index='FT_Nodes', param=('R','X'))
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="P_Load", param='P_fx_Load', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="Q_Load", param='Q_fx_Load', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="P_PV", param='P_PV', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="BESS", index='Nodes', param=('r_batt','r_cvt','P_battmax','Q_battmax','S_battmax','battsize', 'E_min', 'E_max'))
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="P_Shp_Load", index='Nodes', param=('P_shpmax','P_shpmin', 'E_dem'))
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="selectShp", param='selectShp', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="alpha", index='Nodes', param=('alpha'))
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="beta", param='beta', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="Pg_min", param='Pg_min', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="Pg_max", param='Pg_max', format='array')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="eff_in", format='param', param='eff_in')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="eff_out", format='param', param='eff_out')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="dt", format='param', param='dt')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="eps", format='param', param='eps')
data.load(filename="zone2_nonlinear_opt_model.xlsx", range="V", format='param', param='V')
...
model.v = Var(model.Nodes, model.T, bounds=(0.95, 1.05), initialize=1) # in PU
...
model.Q_batt = Var(model.Nodes, model.T)
...
def qbatt_min(model, k, t):
    return (0 <= model.Q_batt[k, t])
model.qbatt_min = Constraint(model.Nodes, model.T, rule = qbatt_min)

def qbatt_max(model, k ,t):
    return (model.Q_batt[k, t] <= model.Q_battmax[k])
model.qbatt_max = Constraint(model.Nodes, model.T, rule = qbatt_max)
...
def bess_balance(model, k, t):
    if t < 2:
        return (model.E_batt[k, t] == model.E_batt[k, t+23] + model.P_bc[k, t] * model.dt * model.eff_in - model.P_bd[k, t] * model.dt/model.eff_out - (((model.r_batt[k] + model.r_cvt[k])/(model.V)**2)*((model.P_batt[k, t])**2)*model.dt) - (((model.r_cvt[k])/(model.V)**2)*((model.Q_batt[k, t])**2)*model.dt))
    return (model.E_batt[k, t] == model.E_batt[k, t-1] + model.P_bc[k, t] * model.dt * model.eff_in - model.P_bd[k, t] * model.dt/model.eff_out - (((model.r_batt[k] + model.r_cvt[k])/(model.V)**2)*((model.P_batt[k, t])**2)*model.dt) - (((model.r_cvt[k])/(model.V)**2)*((model.Q_batt[k, t])**2)*model.dt))
model.bess_balance = Constraint(model.Nodes, model.T, rule = bess_balance)

def bess_nl_cons(model, k, t):
    return ((model.P_batt[k, t])**2 + (model.Q_batt[k, t])**2 <= (model.S_battmax[k])**2)
model.bess_nl_cons = Constraint(model.Nodes, model.T, rule = bess_nl_cons)
...
filepath = 'zone2_nonlinear_opt_data.xlsx'
